...
/// <summary>
/// The authoritative read side of the business information. The query is a plain EF Core query by the
/// canonical key, so Npgsql translates it and every answer comes from the current stored row.
/// </summary>
internal sealed class BusinessInfoReader(StorefrontDbContext dbContext) : IBusinessInfoReader
{
        ...
        CancellationToken cancellationToken)
    {
        return await dbContext.BusinessInfo
            .AsNoTracking()
            .Where(info => info.Key == canonicalKey)
